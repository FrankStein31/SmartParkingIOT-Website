...
void penjadwalan(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
  ...
  char prevOperasionalMotor[5];
  strcpy(prevOperasionalMotor, receivedJadwal.operasionalMotor);
  ...
  if (len == sizeof(receivedJadwal)) {
    ...
    if (strcmp(receivedJadwal.operasionalMotor, prevOperasionalMotor) != 0) {
        Serial.println("DEBUG: Status operasional MOTOR berubah.");
    }
    
    if (strcmp(receivedJadwal.operasionalMotor, "off") == 0) {
        Serial.println("DEBUG: Operasional OFF untuk Motor terdeteksi. Menutup akses.");
        Serial.println("DEBUG: Sensor infrared TIDAK AKAN bereaksi.");
        servoMasuk.write(0); 
        servoKeluar.write(90); 
    } else { 
        Serial.println("DEBUG: Operasional ON untuk Motor. Mengaktifkan akses.");
...
bool readBlock(byte blockNum, byte *buffer, byte *size) {
  ...
  byte actualBytesRead = *size; 
  status = rfid.MIFARE_Read(blockNum, buffer, &actualBytesRead); 
...
void logikaKetersediaanSlotParkir() {
  if (strcmp(receivedJadwal.operasionalMotor, "on") == 0) {
    ...
    if (!isEntryDetected) { 
        ...
        String line2_display_text = "Masuk "; 
        
        if (slotStatusChanged || timeForDefaultRefresh || currentLcdMode != LCD_MODE_SLOTS_AVAILABLE) {
            updateLcdDisplay(line1_display, -1, line2_display_text, 0, LCD_MODE_SLOTS_AVAILABLE);
...
void logikaMasukParkir() {
  if (strcmp(receivedJadwal.operasionalMotor, "on") == 0) {
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
      ...
      bool suksesBacaNama = readBlock(blockNama, buffer, &bufferSize); 
      if (suksesBacaNama) {
      ...
      bool suksesBacaNIM = readBlock(blockNIM, buffer, &bufferSize);
      if (suksesBacaNIM) {
      ...
      bufferSize = sizeof(buffer); 
      bool suksesBacaJurusan = readBlock(blockJurusan, buffer, &bufferSize);
      ...
      servoMasuk.write(90); 
      Serial.println("Gerbang Masuk: Buka (90 derajat)");
      ...
      Serial.println("Gerbang Masuk: Tutup (0 derajat)");
...
void logikaKeluarParkir() {
  if (strcmp(receivedJadwal.operasionalMotor, "on") == 0) {
    if (digitalRead(PIN_IR_KELUAR) == LOW && !isEntryDetected) { 
        ...
        servoKeluar.write(0); 
        Serial.println("Gerbang Keluar: Buka (0 derajat)");
        delay(2000); 
        ...
        Serial.println("Gerbang Keluar: Tutup (90 derajat)");
        delay(1000); 
        currentLcdMode = LCD_MODE_SLOTS_AVAILABLE;
    }
  } else {
